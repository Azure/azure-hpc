...
BlastResultsLoader.prototype._fetch_remote_results = function(blast_results_name, on_fetched) {
  ...
  $.get('/Areas/Kablammo/Content/data/' + blast_results_name, function (xml_doc) {
...
BlastResultsLoader.prototype._fetch_api_results = function (search_id, query_id, blast_results_name, on_fetched) {
    var self = this;
    var cacheKey = search_id + '-' + query_id + '-' + blast_results_name;
    if (typeof _blast_results_cache[cacheKey] !== 'undefined') {
        var results = _blast_results_cache[cacheKey];
        on_fetched(results);

        return;
    }

    $.get('/api/searches/' + search_id + '/queries/' + query_id + '/outputs/' + blast_results_name + '/', function (xml_doc) {
        var blah = $.parseXML(xml_doc);
        var blast_results = self._parser.parse_blast_results(blah);
        _blast_results_cache[cacheKey] = blast_results;
        on_fetched(blast_results);
    }).fail(function (jqxhr, text_status, error) {
        Interface.error('Failed to retrieve ' + blast_results_name + ': ' + error);
    });
...
BlastResultsLoader.prototype.load_from_api = function (search_id, query_id, blast_results_name, on_done) {
    var self = this;
    this._fetch_api_results(search_id, query_id, blast_results_name, function (blast_results) {
        // Don't munge earlier, as we don't want to cache the sliced-and-diced
        // version.
        self._munge_results(blast_results);
        on_done(blast_results);
