...
BlastParser.prototype._filter_by_thresholds = function(iterations) {
  var evalue_threshold = parseFloat($('#evalue-threshold').val()) || 0;
  var bitscore_threshold = parseFloat($('#bitscore-threshold').val()) || 0;
  var coverage_threshold = (parseFloat($('#hsp-coverage-threshold').val()) || 10) / 100;
...
BlastParser.prototype._slice_blast_iterations = function(iterations) {
  // TODO: eliminate dependence on DOM. Pass filter values as parameters.
  var max_query_seqs = parseInt($('#max-query-seqs').val(), 10) || 10;
  var max_hits_per_query_seq = parseInt($('#max-subjects-per-query-seq').val(), 10) || 5;
